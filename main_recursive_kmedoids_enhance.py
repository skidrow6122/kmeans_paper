    ...
    for i in groupKrange :
        ...
        if term == "recursive" :
            ...
            for j in tempK:
                ...
                KmedoidsModel2 = KMedoids(metric="cosine", n_clusters=j).fit(df_temp[['tolatitude', 'tolongitude']])
                inertias.append(KmedoidsModel2.inertia_)
...
def clustering (k, target, term) :
    ...
    KmedoidsModel = KMedoids(metric="cosine", n_clusters=k).fit(target[['tolatitude', 'tolongitude']])

    # find the number of data points in each cluster
    clusterCount = []
    dataPoints = KmedoidsModel.predict(target[['tolatitude', 'tolongitude']])
    ...
    if invalidCount >= clusterCount.size/2 :
        ...
        KmedoidsModel.labels_
        target['result'] = KmedoidsModel.labels_
